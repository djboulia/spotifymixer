...
var PlayList = function (spotifyApi) {
  ...
  async function doTracksRetries(spotifyApi, playListId) {
    ...
    for (var i = 0; i < 10; i++) {
      ...
      const data = await retryTracks(spotifyApi, RETRY_DELAY, playListId)
        .catch((err) => {
   ...
   */
   async function doReorderRetries(spotifyApi, playListId, from, to, snapshot_id) {
    ...
    for (var i = 0; i < 10; i++) {
      ...
      const data = await retryReorderTrack(spotifyApi, RETRY_DELAY, playListId, from, to, snapshot_id)
        .catch((err) => {
          console.log('retry error! ', err);
          finished = false;
        });
