...
export const ProgressStats = ({
  ...
  if (!categories || categories?.length === 0) {
    return <Alert severity="info" message="Loading categories..." />;
  }
  ...
  return (
    <div className="m-2">
      <div className="max-h-[200px] min-h-[200px] min-w-[350px] overflow-y-auto px-4 md:min-w-[400px]">
        <div className="border-spotify-700 flex flex-row justify-between border-b py-2">
        ...
        {noSingles.map((category, idx) => (
          <div
            key={idx}
            className="border-spotify-700 flex flex-row justify-between border-b py-2"
          >
            <div className="w-[200px] truncate text-left text-ellipsis">
              {category.category}
            </div>
